...
from configs.config import EncoderConfig, DecoderConfig, PaliGemmaConfig
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        if position_ids is None:
            ...
            ).unsqueeze(0).expand(x.shape[0], -1)  # [B, Seq_Len]
...
class GemmaAttention(nn.Module):
    ...
    def __init__(self, config: DecoderConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class PaliGemmaForConditionalGeneration(nn.Module):
    def __init__(self, config: PaliGemmaConfig):
        super().__init__()
        self.config = config
        self.vision_tower = CLIPWithLoRA(config.vision_config)
    ...
    def _merge_input_ids_with_image_features(
        ...
        # print("text_mask_expanded shape: ", text_mask_expanded.shape)
        ...
        # image_mask: [B, T] is True where <image> appears 
        ...
        if kv_cache is not None and kv_cache.num_items() > 0:
            ...
            if attention_mask is not None:
                ...
                position_ids = torch.arange(final_embedding.shape[1], device=final_embedding.device).unsqueeze(0)
    ...
    def forward(
        ...
        # Flatten image pairs to batch format for vision encoder
        B, N, C, H, W = pixel_values.shape  # N=2 images per sample
