...
class BaseDataset(Dataset):
    def __init__(self, tokenizer, split, config=DataConfig(), preprocess=None):
        ...
        bert_embeds = torch.load(f"cached_embeds/{split}_bert_embeds.pt")

        for i in range(len(self.examples)):
            # === CLEAN REPORT BEFORE TOKENIZATION ===
            report = self.clean_report_iu_xray(self.examples[i]['report'])

            # === TOKENIZE CLEANED REPORT ===
            inputs = tokenizer(report, return_tensors="pt", padding=True, truncation=True)
            report_ids = inputs["input_ids"].tolist()[0]

            # === COMPOSE FINAL INPUT IDS + MASK ===
            ids = [image_token_id] * self.num_image_tokens + report_ids
            mask = [0] * self.num_image_tokens + [1] * len(report_ids)

            self.examples[i]['ids'] = ids
            self.examples[i]['mask'] = mask
            self.examples[i]["bert"] = bert_embeds[i]
...
class Iuxray(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        return (image_id, image, example['ids'], example['mask'], example['bert'], len(example['ids']))
...
class CustomDataLoader(DataLoader):
    ...
    def collate_fn(data, pad_token_id=0, max_text_len=100, num_image_tokens=100):
        input_ids, images_features, reports_ids, reports_masks, input_embeds, seq_lengths = zip(*data)
        images_features = torch.stack(images_features, 0)
        input_embeds = torch.stack(input_embeds, 0)
        ...
        return targets, images_features, input_ids, targets_masks, input_embeds